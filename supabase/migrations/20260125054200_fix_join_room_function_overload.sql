-- Fix: Remove duplicate join_room_by_code function overload
-- The old function signature (without password) still exists, causing ambiguity.

-- 1. Drop the old function signature (no password parameter)
drop function if exists public.join_room_by_code(text);

-- 2. Recreate the function with the password parameter (idempotent)
